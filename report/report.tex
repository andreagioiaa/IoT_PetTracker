...
I dati inviati dal dispositivo transitano inizialmente nella collezione di staging \texttt{data\_sent\_raw}, 
...
\texttt{v} (vehicle)  & \texttt{a}           & False                   & False             & In viaggio su veicolo.        \\ 
\texttt{r} (robbed)   & \texttt{q}           & True                    & False             & Rischio di furto su veicolo.  \\
...
\begin{itemize}
    ...
    \item \textbf{Split notturno (\texttt{23:59} ora italiana):} suddivide le sessioni operative che si protraggono a cavallo di due giornate, al fine di non alterare le statistiche grafiche giornaliere visibili all'utente. Poiché i \textit{timestamp} sono memorizzati nativamente in UTC, il sistema gestisce dinamicamente l'alternanza tra ora solare (CET, 22:59:59 UTC) e ora legale (CEST, 21:59:59 UTC) tramite la funzione \textit{helper} \texttt{getItalyTime()}, evitando l'impiego di \textit{offset} temporali cablati a codice (\textit{hardcoded}). Il \textit{cron job} è pertanto schedulato su entrambi gli orari critici UTC (\texttt{59 21,22 * * *}), delegando a un controllo interno la soppressione dell'esecuzione non pertinente alla stagione in corso. In fase di elaborazione, le sessioni in stato di \textit{sleep} vengono preventivamente riattivate per garantire l'integrità dello storico, mentre le occorrenze già marcate come anomale (\texttt{anomaly = true}) vengono ignorate, impedendone la duplicazione nel giorno successivo.
\end{itemize}
