...
\begin{itemize}
    ...
    \item \textbf{Modellazione del comportamento dell'animale:} definire una logica in grado di distinguere in modo affidabile le diverse attività dell'animale, riducendo i falsi positivi dovuti al rumore dei sensori.
    \item \textbf{Sviluppo di un sistema di allerta:} implementare un meccanismo di notifiche capace di avvisare l'utente in caso di uscita dall'area di geofence (geofencing \footnote{Un geofence è un perimetro virtuale associato ad un'area geografica del mondo reale. Può essere generato dinamicamente, ad esempio come l'area entro un raggio da un punto prefissato, oppure può essere definito da un insieme di confini prestabiliti.}) o di condizioni critiche del dispositivo.
    ...
    \item \textbf{Gestione multi-utente:} garantire la gestione di più dispositivi mobili per singolo utente. Inoltre, ogni utente avrà la possibilità di creare le proprie zone di geofence e possedere l'applicazione su più dispositivi, funzionando in maniera sincronizzata.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Rilevamento attività:} il firmware monitora costantemente i passi ricevuti ad ogni pacchetto,
          azzerando a ogni nuovo movimento il timer di inattività (\texttt{lastActivityTime}) per rilevare anomalie o la chiusura dell'attività in corso.
...
La logica di geofencing è implementata dal lato client tramite algoritmo \mbox{\textit{Ray-Casting}}. 
...
e permette di gestire le funzionalità visive in tempo reale, in totale autonomia dal leggere le notifiche di attività dal \textit{backend}.
...
Sebbene il prototipo realizzato soddisfa i requisiti prefissati, alcune scelte progettuali 
sono state guidate da vincoli di tempo e risorse tipici di un contesto universitario. 
Di seguito vengono proposte alcune direzioni di evoluzione, sia a livello di backend che di hardware.
...
\section{Conclusioni}
Il progetto Pet Tracker ha raggiunto con successo gli obiettivi prefissati in fase di progettazione. È stato sviluppato un sistema end-to-end completo, dal dispositivo embedded indossabile fino all'applicazione mobile, capace di monitorare in tempo reale la posizione e l'attività motoria dell'animale domestico. Inoltre, il sistema controlla costantemente lo stato energetico del dispositivo, garantendo elevati standard di efficienza e sicurezza.

A livello di firmware e backend, l'adozione di PocketBase si è dimostrata una scelta efficace per abbattere i tempi di sviluppo. Tuttavia, questa tecnologia ha imposto sfide tecniche significative, tra cui l'incompatibilità tra il motore Goja e l'SDK di Firebase, una criticità che è stata superata con successo implementando un microservizio bridge dedicato.

Il sistema di notifiche push, pur richiedendo un'architettura a bridge per aggirare i limiti del runtime server-side, segnala tempestivamente gli eventi rilevanti per l'utente rendendo il prodotto realmente utilizzabile in scenari reali di sorveglianza dell'animale.

I test condotti sul campo hanno fatto emergere alcune limitazioni del sistema. Tuttavia, il progetto dimostra concretamente come i paradigmi dell'IoT (acquisizione, trasmissione a basso consumo, elaborazione server-side e fornitura di dati strutturati al client) possano essere applicati con successo a un caso d'uso di notevole utilità pratica. Il lavoro svolto pone, inoltre, basi solide e ben documentate per la futura evoluzione verso un'architettura pienamente scalabile. Infatti, in un futuro il sistema potrà essere facilmente esteso per supportare un numero maggiore di dispositivi, migliorare la robustezza del backend e ottimizzare l'hardware per renderlo più compatto e resistente.
